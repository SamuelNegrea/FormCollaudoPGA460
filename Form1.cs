...
namespace FormCollaudoPGA460
{
    partial class Form1 : Form
    {
        ...
        private void EncodeRegisters14To6E(bool writeEepromControl = false)
        {
            ...
            EncodeTVGGainValues();
            EncodeMainConfig();
            if (writeEepromControl) EncodeEepromControl();
            EncodeFilterConfig();
            EncodeP1Threshold();
        }
        ...
        private void EncodeTVGGainValues()
        {
            ...
            byte reg1A = (byte)(
                (ReadRegister(0x1A) & 0x02) | ((fields.TVG_G5 & 0x3F) << 2) | (fields.FREQ_SHIFT & 0x01));
        ...
        {
            byte reg40 = (byte)(
                (virtualRegs[0x40] & 0x04) | ((fields.DATADUMP_EN & 0x01) << 7) | ((fields.EE_UNLCK & 0x0F) << 3) | ((fields.EE_RLOAD & 0x01) << 1) | (fields.EE_PRGM & 0x01));
        ...
        private void EncodeFilterConfig()
        {
            ...
            byte reg4B = (byte)(
                (virtualRegs[0x4B] & 0x10) | ((fields.TEST_MUX & 0x07) << 5) | ((fields.SAMPLE_SEL & 0x01) << 3) | (fields.DP_MUX & 0x07));
        ...
        public Form1()
        {
            ...
            numBurstInterval.ValueChanged += NumBurstInterval_ValueChanged;
        }
        ...
        private void btnReadConfig_Click(object sender, EventArgs e)
        {
            try
            {
                if (offlineMode) GridToVirtualRegisters();
        ...
        private void btnWriteConfig_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (offlineMode) VirtualRegistersToGrid();
        ...
        {
            if (offlineMode) return ReadVirtualRegister(addr);
            if (serial == null || !serial.IsOpen) throw new Exception("Connettere prima la porta COM");
            ...
            if (available == 0) throw new Exception("Nessuna risposta");
            byte[] rx = new byte[available];
            serial.Read(rx, 0, available);
            if (rx.Length < 3) throw new Exception("Risposta incompleta");
            byte value = rx[1];
            virtualRegs[addr] = value;
            return value;
        }
        ...
        private void InitFieldsGrid()
        {
            ...
            fieldInfos.Add("TH_P1_L9", new FieldInfoEx("preset 1 Livello trigger 9    DEVE ESSERE INIZIALIZZATO", "0", "32"));
            fieldInfos.Add("TH_P1_L10", new FieldInfoEx("preset 1 Livello trigger 10    DEVE ESSERE INIZIALIZZATO", "0", "32"));
            fieldInfos.Add("TH_P1_L11", new FieldInfoEx("preset 1 Livello trigger 11    DEVE ESSERE INIZIALIZZATO", "0", "32"));
            fieldInfos.Add("TH_P1_L12", new FieldInfoEx("preset 1 Livello trigger 12    DEVE ESSERE INIZIALIZZATO", "0", "32"));
        ...
        private void UpdateFieldsGrid()
        {
            foreach (DataGridViewRow row in dgvFields.Rows)
            {
                ...
                if (f != null) row.Cells[1].Value = ((byte)f.GetValue(fields)).ToString();
            }
        }
        private void ReadFieldsGrid()
        {
            foreach (DataGridViewRow row in dgvFields.Rows)
            {
                ...
                FieldInfo f = typeof(PGA460Fields).GetField(name);
                if (f != null) f.SetValue(fields, Convert.ToByte(text));
            }
        }
        ...
        private void btnLeggi_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (available == 0)
                {
                    MessageBox.Show("Nessuna risposta dal PGA460");
                    return;
                }
                byte[] rx = new byte[available];
                serial.Read(rx, 0, available);
                MessageBox.Show("RX: " + BitConverter.ToString(rx));
                if (rx.Length >= 3) txtValore.Text = rx[1].ToString("X2");
            }
            catch (Exception ex)
            {
        ...
        private void btnScan_Click(object sender, EventArgs e)
        {
            ...
            btnScan.Text = scanEnabled ? "SCAN ON" : "SCAN OFF";
        }
        private void ScanTimer_Tick(object sender, EventArgs e)
        {
            /*scanTimer.Enabled = false;
            scanEnabled = false;
            btnScan.Text = "SCAN OFF";*/
            // 1) Invia burst
            SendBurst();
            int TL = Convert.ToInt32(txtTL.Text);
            System.Threading.Thread.Sleep(TL);
            // 2) Leggi misura distanza
    ...
    BitConverter.ToString(rx));*/
        ...
        private void DrawDump(byte[] samples)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                List<PointF> thrPts = BuildThresholdCurve(area);
                ...
                List<PointF> gainPts = BuildTVGCurve(area);
        ...
        private List<PointF> BuildThresholdCurve(Rectangle area)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                float y = area.Bottom - (L[i] * area.Height / 240f);
